...
class PocketChunkParser{
  ...
  public function getParsedChunk($x,$z) {
	...
	private function loadLocationTable(){
		$this->location = array();
		echo(" Loading Chunk Location table...");
	...
	public function saveMap($final = false){
	  echo("Saving chunks...\n");
		...
		echo("Saved chunks.dat.gz with ".round(($compressed/$original)*100, 2)."% (".round($compressed/1024, 2)."KB) of the original size\n");
		if($final === true){
			@unlink($this->file);
		}
	}
	...
	public function getBlockSkyLight($x, $y, $z){
		...
		$aZ = $z - ($Z << 4);
		$index = $aZ + ($aX << 4);
		$lite = ord($this->map[$X][$Z][2][$index]{$y >> 1});
		if(($y & 1) === 0){
			$lite = $lite & 0x0F;
		}else{
			$lite = $lite >> 4;
	...
	public function getBlockLight($x, $y, $z){
		...
		$aZ = $z - ($Z << 4);
		$index = $aZ + ($aX << 4);
		$lite = ord($this->map[$X][$Z][3][$index]{$y >> 1});
		if(($y & 1) === 0){
			$lite = $lite & 0x0F;
		}else{
			$lite = $lite >> 4;
