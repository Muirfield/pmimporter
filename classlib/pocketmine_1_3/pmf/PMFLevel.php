<?php
namespace pocketmine_1_3\pmf;
use pmsrc\utils\Binary as Utils;
use pocketmine_1_3\pmf\PMF;
/**
 *
...
class PMFLevel extends PMF{
  ...
  const PMF_CURRENT_LEVEL_VERSION = 0x00;
	...
	public function saveData($locationTable = true){
	  $this->levelData["version"] = self::PMF_CURRENT_LEVEL_VERSION;
		@ftruncate($this->fp, 5);
		$this->seek(5);
		...
		$extra = gzdeflate($this->levelData["extra"], self::PMF_LEVEL_DEFLATE_LEVEL);
	...
	private function createBlank(){
		...
		for($index = 0; $index < $cnt; ++$index){
			...
			@file_put_contents($this->getChunkPath($X, $Z), gzdeflate("", self::PMF_LEVEL_DEFLATE_LEVEL));
		}
	...
	protected function parseLevel(){
		...
		if($this->levelData["version"] > self::PMF_CURRENT_LEVEL_VERSION){
			return false;
		}
		...
		if(($len = $this->read(2)) === false or ($this->levelData["extra"] = @gzinflate($this->read(Utils::readShort($len, false)))) === false){ //Corruption protection
			echo("[NOTICE] Empty/corrupt location table detected, forcing recovery\n");
		...
		for($Y = 0; $Y < $this->levelData["height"]; ++$Y){
			...
			if(($info[0] & $t) === $t){
				...
				if(strlen($this->chunks[$index][$Y] = gzread($chunk, 8192)) < 8192){
					echo("[NOTICE] Empty corrupt chunk detected [$X,$Z,:$Y], recovering contents\n");
  ...
  public function getBlockDamageColumn($x,$z) {
	...
	public function saveChunk($X, $Z){
		...
		$chunk = @gzopen($this->getChunkPath($X, $Z), "wb".self::PMF_LEVEL_DEFLATE_LEVEL);
		$bitmap = 0;
		for($Y = 0; $Y < $this->levelData["height"]; ++$Y){
